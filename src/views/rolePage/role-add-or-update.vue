	...
	>
		<el-form :model="dataForm" ref="dataForm" :rules="dataRule" label-width="80px">
			<el-form-item label="名称" prop="name">
				<el-input v-model="dataForm.name" size="medium" clearable :disabled="type==='update'"/>
			</el-form-item>
      ...
      <el-form-item label="描述" prop="desc">
        <el-input v-model="dataForm.desc" size="medium" clearable :disabled="type==='update'"/>
      </el-form-item>
		</el-form>
		<template #footer>
...
export default {
	data: function() {
      ...
      permissionList:null,
			dataRule: {
				name: [{ required: true, pattern: '^.{1,20}$', message: '用户名格式错误' }],
	...
	methods: {
      ...
      that.type = id?'update':'add';
			...
			that.$nextTick(() => {
				...
				if (that.dataForm.id) {
					that.$http('user/role', 'POST', { id: id }, true, function(resp) {
						that.dataForm.name = resp.name;
						that.dataForm.permissions = resp.permissions;
